...
export default {
  ...
  setup(_props) {
    const itemsRef = ref([]);
    const coolLightbox = ref(null);
    return { itemsRef, coolLightbox };
  },
  directives: {
    ...
    zoomBar(newVal, prevVal) {
      ...
      if (this.isZooming) {
        if (this.effect == "swipe") {
          item = this.itemsRef[this.imgIndex].childNodes[0];
        } else {
          item = this.itemsRef.childNodes[0];
        }
      ...
      if (prev !== null) {
        ...
        if (this.enableScrollLock) {
          this.$nextTick(() => {
            ...
            disableBodyScroll(self.coolLightbox);
          });
          // setTimeout(function() {
          // }, 50);
        }
        ...
        if (this.enableScrollLock) {
          ...
          enableBodyScroll(self.coolLightbox);
        }
  ...
  beforeUnmount() {
    if (this.enableScrollLock) {
      ...
      if (this.coolLightbox) {
        enableBodyScroll(this.coolLightbox);
      }
    }
  },
  beforeUpdate() {
  ...
  methods: {
    ...
    checkIfIsButton(event) {
      var elements =
        ".cool-lightbox__iframe *, .cool-lightbox-button, .cool-lightbox-button *, .cool-lightbox-toolbar__btn, .cool-lightbox-toolbar__btn *, .cool-lightbox-caption h6, .cool-lightbox-caption p, .cool-lightbox-caption a";
    ...
    handleMouseMove(e) {
      ...
      if (this.isDraging) {
        ...
        const item =
          e.target.parentNode.nodeName === "PICTURE"
    ...
    zoomImage(indexImage) {
      ...
      if (this.isZooming) {
        ...
        // add scale
        item.style.transform =
          "translate3d(calc(-50%), calc(-50%), 0px) scale3d(1.6, 1.6, 1.6)";
    ...
    setXPosition(index) {
      ...
      if (this.dir === "rtl") {
        this.xSwipeWrapper =
          index * this.lightboxInnerWidth + rightMargin * index + rightMargin;
        return;
      }
      // set x position
      this.xSwipeWrapper =
        -index * this.lightboxInnerWidth - rightMargin * index;
      return;
    },
