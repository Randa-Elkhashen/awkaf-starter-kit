  ...
  Widget build(BuildContext context) {
    ...
    if(["png",'jpg'].contains(url.split(".").last)) {
      _child = SizedBox.expand(
          child: ImageLoader(url: url,boxFit: BoxFit.cover,)
      );
    }else{
      ...
      if(_uri.host ==  "www.soundcloud.com" || _uri.host == "soundcloud.com"){
        _child = Container(
          color: Colors.orange,
          child: const Center(child: FaIcon(FontAwesomeIcons.soundcloud,color: Colors.white,)),
        );
      } else if(_youTubeId != null){
        _child = Stack(
          fit: StackFit.expand,
          children: [
            ImageLoader(url: "https://img.youtube.com/vi/$_youTubeId/0.jpg",
              boxFit: BoxFit.cover,),
            const Center(child: FaIcon(FontAwesomeIcons.youtube,color: Colors.red,)),
          ],
        );
      }
    }
    return MaterialButton(
      padding: EdgeInsets.all(4.0 * AppStyle.scaleFactor),
      onPressed: onPressed,
